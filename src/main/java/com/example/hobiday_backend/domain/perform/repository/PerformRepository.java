...
//            "where p.genrenm = :genre " +
    ...
    Optional<List<Perform>> findByGenreNm(@Param("genre") String genre);
//            "where p.genrenm like concat('%', :genre, '%')") //+
    ...
    @Query("select p " +
            ...
            "order by p.prfpdfrom asc limit 10")
    Optional<List<Perform>> findByPrfNm(@Param("search") String search);

    // 모든 장르 선택
    @Query("select p " +
            "from Perform p " +
            "where p.genrenm = :genre " +
            "order by p.prfpdfrom asc limit 5")
    Optional<List<Perform>> findAllByGenreNm(@Param("genre") String genre);
